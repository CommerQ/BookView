...
| ID | 需求 | 优先级 | 说明 |
|----|------|--------|------|
| FR-M01 | 按 `section_id` 生成教学动画脚本（MAS） | P1 | 见 [数学动画设计](./2026-05-16-math-chapter-animation-design.md) |
| FR-M02 | 前端 GSAP 解释器播放，禁止 AI 直接写 JS | P0 | 白名单图元 |
| FR-M03 | 动画步骤溯源到 `block_id` | P1 | 与原文对照 |
| FR-M04 | 草稿预览 + 教师发布后学生可见 | P1 | draft / published |
